...
const ExcelUploadComponent: React.FC<ExcelUploadComponentProps> = ({
		...
		reader.onload = (e) => {
			try {
				...
				workbook.SheetNames.forEach(sheetName => {
					...
					const matchingQuestionType = questionTypes.find(qt => 
						qt.name.toLowerCase().includes(sheetName.toLowerCase()) ||
					...
					if (matchingQuestionType && jsonData.length > 1) {
									...
									for (let i = 1; i < jsonData.length; i++) {
										...
										if (row.length >= 3) {
											...
											for (let j = 1; j <= matchingQuestionType.max_answer; j++) {
												if (row[j]) {
											...
											const correctAnswerColumn = matchingQuestionType.max_answer + 1;
											if (row[correctAnswerColumn]) {
												correctAnswer = parseInt(row[correctAnswerColumn].toString()) || 1;
											}
				...
				if (questionTypeData.length === 0) {
					toast.error('No se encontraron tipos de pregunta válidos en el archivo Excel');
				} else {
					toast.success(`Se encontraron ${questionTypeData.length} tipos de pregunta`);
				}
			} catch (error) {
	...
	const handleTestSelection = (questionTypeName: string, testId: number) => {
		setParsedData(prev => prev.map(qtd => 
	...
	const handleUpload = async () => {
		...
		const invalidQuestionTypes = parsedData.filter(qtd => qtd.selectedTest === null);
		if (invalidQuestionTypes.length > 0) {
			toast.error('Por favor selecciona un examen para todos los tipos de pregunta');
			return;
		}
	...
	return (
		...
		>
			<DialogHeader
				...
				onPointerLeaveCapture={undefined}
			>
				<Typography variant="h5" placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
			...
			>
					...
					>
						<div className="flex flex-col items-center space-y-4">
							...
							<Alert
								...
								className="text-sm"
							>
								<strong>Formato esperado:</strong> Cada hoja debe corresponder a un tipo de pregunta. 
								Columnas: Pregunta, Respuesta1, Respuesta2, ..., RespuestaCorrecta (número)
							</Alert>
						</div>
					</CardBody>
				</Card>
				...
				{parsedData.length > 0 && (
					<div className="space-y-4">
						<Typography variant="h6" className="text-center" placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
									...
									<div className="space-y-4">
										<div className="flex items-center justify-between">
											<Typography variant="h6" placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
												{qtData.questionType.name} ({qtData.questions.length} preguntas)
											</Typography>
											<Button
												variant="text"
												size="sm"
												onClick={() => toggleCollapse(qtData.questionType.name)}
												placeholder={undefined}
												onPointerEnterCapture={undefined}
												onPointerLeaveCapture={undefined}
											>
												{collapseStates[qtData.questionType.name] ? 'Ocultar' : 'Mostrar'}
											</Button>
										</div>
										...
										<div className="w-72">
											...
											<Select
												...
												onChange={(value) => handleTestSelection(qtData.questionType.name, parseInt(value!))}
												placeholder={undefined}
												onPointerEnterCapture={undefined}
												onPointerLeaveCapture={undefined}
											>
												{testList.map(test => (
													<Option key={test.id} value={test.id.toString()}>
														{test.code} - {test.status ? 'Activo' : 'Inactivo'}
													</Option>
												))}
											</Select>
										</div>
											...
											<div className="space-y-3 mt-4">
												...
												>
														<CardBody
															...
															onPointerLeaveCapture={undefined}
														>
															<Typography variant="small" className="font-medium mb-2" placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
																Pregunta {qIndex + 1}: {question.header}
															</Typography>
															<div className="space-y-1">
																{question.answers.map((answer, aIndex) => (
																	<div key={aIndex} className="flex items-center space-x-2">
																		{question.correctAnswer === aIndex + 1 && (
																			<Check size={16} className="text-green-600" />
																		)}
																		<Typography
																			variant="small"
																			className={question.correctAnswer === aIndex + 1 ? 'text-green-600 font-medium' : ''}
																			placeholder={undefined} 
																			onPointerEnterCapture={undefined} 
																			onPointerLeaveCapture={undefined}
																		>
