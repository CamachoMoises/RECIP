...
const ViewCourseStudentSchedule = () => {
	...
	useEffect(() => {
		const loadData = async () => {
			...
			if (courseId > 0 && courseStudentId > 0) {
				await Promise.all([
					dispatch(fetchSubjects({ course_id: courseId, status: true, is_schedulable: true })),
	...
	const studentFromList = studentFromCourse?.id
		? user.studentList.find((p) => p.student?.id === studentFromCourse?.id)
		: null;
	...
	if (course.status === 'loading' || !dataLoaded) return <LoadingPage />;
								...
								<div className="space-y-3">
									<div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
										...
										<div>
											...
											<Typography
												...
												onPointerLeaveCapture={undefined}
											>
												{studentFromCourse?.user?.name || studentFromList?.name} {studentFromCourse?.user?.last_name || studentFromList?.last_name}
											</Typography>
										</div>
									</div>
									...
									<div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
										...
										<Typography
											...
											onPointerLeaveCapture={undefined}
										>
											{studentFromCourse?.user?.doc_number || studentFromList?.doc_number}
										</Typography>
									</div>
									...
									<div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
										...
										<Typography
											...
											onPointerLeaveCapture={undefined}
										>
											{studentFromCourse?.user?.phone || studentFromList?.phone}
										</Typography>
									</div>
								</div>
							)}
									...
									<div className="flex flex-col gap-2">
										<div className="p-2 rounded bg-gray-100">
											<Typography
												...
												onPointerLeaveCapture={undefined}
											>
												{getTypeTripLabel(course.courseStudent?.type_trip)}
											</Typography>
										</div>
									</div>
								</div>
																	...
																	<div className="flex-1">
																		...
																		{schedule ? (
																			<Typography
																				...
																				onPointerLeaveCapture={undefined}
																			>
																				Horas: {schedule.classTime} / {subjectItem.hours}
																				hrs
																			</Typography>
																		) : (
																			<Typography
